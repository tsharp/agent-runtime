use agent_runtime::{
    AgentConfig, Runtime, Workflow, AgentStep, TransformStep, ConditionalStep,
    tool::CalculatorTool,
};
use std::sync::Arc;
...
async fn main() {
    ...
    let extract_step = TransformStep::new(
    ...
    let positive_transform = TransformStep::new(
        "positive_handler".to_string(),
        |data| {
    ...
    let negative_transform = TransformStep::new(
        "negative_handler".to_string(),
        |data| {
            let val = data.get("value").and_then(|v| v.as_i64()).unwrap_or(0);
            serde_json::json!({
                "value": val,
    ...
    let extract_step2 = TransformStep::new(
        "extract_number".to_string(),
        |data| {
            serde_json::json!({
                "value": data.get("number").and_then(|v| v.as_i64()).unwrap_or(0)
    ...
    let positive_transform2 = TransformStep::new(
        "positive_handler".to_string(),
        |data| {
            let val = data.get("value").and_then(|v| v.as_i64()).unwrap_or(0);
            serde_json::json!({
    ...
    let negative_transform2 = TransformStep::new(
        "negative_handler".to_string(),
        |data| {
            let val = data.get("value").and_then(|v| v.as_i64()).unwrap_or(0);
            serde_json::json!({
