...
impl OrchestratorContext {
    fn new (chat_history: Option<Vec<ChatMessage>>) -> Self {
        Self { 
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut context = OrchestratorContext::new(Some(vec![
        ChatMessage::user("What is the capital of France?"),
    ]));

    println!("[{}] Sending two consecutive user messages:", ts());
    for (i, msg) in context.chat_history.iter().enumerate() {
    ...
    let input = AgentInput {
        ...
        chat_history: Some(context.chat_history.clone()),
    };
