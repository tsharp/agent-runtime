...
async fn test_tool_execution_failure() {
        ...
        |_args| {
            Box::pin(async move {
                Err(ToolError::ExecutionFailed(
                    "Tool failed intentionally".into(),
                ))
            })
        },
    ));
    ...
    assert!(
        result.is_ok(),
        "Agent should handle tool failure: {:?}",
        result.err()
    );
}
...
async fn test_malformed_tool_call_arguments() {
    ...
    let mut registry = ToolRegistry::new();
    registry.register(NativeTool::new("test_tool", "Test", json!({}), |args| {
        Box::pin(async move {
            let start = std::time::Instant::now();
...
async fn test_empty_tool_calls_array() {
    ...
    let mock_client = MockLlmClient::new().with_response("Response without tools");
...
async fn test_concurrent_tool_failures() {
    ...
    for i in 1..=3 {
        ...
        registry.register(NativeTool::new(
            ...
            move |_args| {
                Box::pin(
                    async move { Err(ToolError::ExecutionFailed(format!("Tool {} failed", i))) },
                )
            },
        ));
    }
