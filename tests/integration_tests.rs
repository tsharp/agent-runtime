    ...
    NativeTool::new(
        ...
        |args| Box::pin(async move {
            ...
            let result = match op {
                ...
                "multiply" => a * b,
                "divide" if b != 0.0 => a / b,
                "divide" => return Ok(ToolResult::error("Division by zero", start.elapsed().as_secs_f64() * 1000.0)),
                _ => return Ok(ToolResult::error("Unknown operation", start.elapsed().as_secs_f64() * 1000.0)),
            };

            Ok(ToolResult::success(json!({"result": result}), start.elapsed().as_secs_f64() * 1000.0))
        })
    )
}
    ...
    NativeTool::new(
        ...
        |args| Box::pin(async move {
            ...
            if query.contains("rust") {
                ...
                Ok(ToolResult::success_no_data(format!("No results found for '{}'", query), start.elapsed().as_secs_f64() * 1000.0))
            }
        })
    )
}
